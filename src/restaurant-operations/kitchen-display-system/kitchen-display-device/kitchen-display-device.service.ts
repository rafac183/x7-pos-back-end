...
import {
  KitchenDisplayDeviceResponseDto,
  OneKitchenDisplayDeviceResponseDto,
} from './dto/kitchen-display-device-response.dto';
...
export class KitchenDisplayDeviceService {
  ...
  async create(
    createKitchenDisplayDeviceDto: CreateKitchenDisplayDeviceDto,
    authenticatedUserMerchantId: number,
  ): Promise<OneKitchenDisplayDeviceResponseDto> {
    ...
    if (
      !createKitchenDisplayDeviceDto.name ||
      createKitchenDisplayDeviceDto.name.trim().length === 0
    ) {
    ...
    if (createKitchenDisplayDeviceDto.deviceIdentifier.length > 100) {
      throw new BadRequestException(
        'Device identifier cannot exceed 100 characters',
      );
    }
    ...
    kitchenDisplayDevice.station_id =
      createKitchenDisplayDeviceDto.stationId || null;
    kitchenDisplayDevice.name = createKitchenDisplayDeviceDto.name.trim();
    kitchenDisplayDevice.device_identifier =
      createKitchenDisplayDeviceDto.deviceIdentifier;
    kitchenDisplayDevice.ip_address =
      createKitchenDisplayDeviceDto.ipAddress || null;
    kitchenDisplayDevice.is_online =
      createKitchenDisplayDeviceDto.isOnline ?? false;
    kitchenDisplayDevice.last_sync =
      createKitchenDisplayDeviceDto.lastSync || null;

    const savedKitchenDisplayDevice =
      await this.kitchenDisplayDeviceRepository.save(kitchenDisplayDevice);

    const completeKitchenDisplayDevice =
      await this.kitchenDisplayDeviceRepository.findOne({
    ...
    if (!completeKitchenDisplayDevice) {
      throw new NotFoundException(
        'Kitchen display device not found after creation',
      );
    }
    ...
    return {
      ...
      data: this.formatKitchenDisplayDeviceResponse(
        completeKitchenDisplayDevice,
      ),
    };
  }

  async findAll(
    query: GetKitchenDisplayDeviceQueryDto,
    authenticatedUserMerchantId: number,
  ): Promise<PaginatedKitchenDisplayDeviceResponseDto> {
    if (!authenticatedUserMerchantId) {
      throw new ForbiddenException(
    ...
    if (query.name) {
      queryBuilder.andWhere('kitchenDisplayDevice.name ILIKE :name', {
        name: `%${query.name}%`,
      });
    }
    ...
    if (query.deviceIdentifier) {
      queryBuilder.andWhere(
        'kitchenDisplayDevice.device_identifier = :deviceIdentifier',
        { deviceIdentifier: query.deviceIdentifier },
      );
    }

    if (query.ipAddress) {
      queryBuilder.andWhere('kitchenDisplayDevice.ip_address = :ipAddress', {
        ipAddress: query.ipAddress,
      });
    }

    if (query.isOnline !== undefined) {
      queryBuilder.andWhere('kitchenDisplayDevice.is_online = :isOnline', {
    ...
    if (query.createdDate) {
      ...
      queryBuilder
        .andWhere('kitchenDisplayDevice.created_at >= :startDate', {
          startDate,
        })
    ...
    const sortField =
      query.sortBy === KitchenDisplayDeviceSortBy.STATION_ID
        ? 'kitchenDisplayDevice.station_id'
        : query.sortBy === KitchenDisplayDeviceSortBy.NAME
          ? 'kitchenDisplayDevice.name'
          : query.sortBy === KitchenDisplayDeviceSortBy.DEVICE_IDENTIFIER
            ? 'kitchenDisplayDevice.device_identifier'
            : query.sortBy === KitchenDisplayDeviceSortBy.IP_ADDRESS
              ? 'kitchenDisplayDevice.ip_address'
              : query.sortBy === KitchenDisplayDeviceSortBy.IS_ONLINE
                ? 'kitchenDisplayDevice.is_online'
                : query.sortBy === KitchenDisplayDeviceSortBy.LAST_SYNC
                  ? 'kitchenDisplayDevice.last_sync'
                  : query.sortBy === KitchenDisplayDeviceSortBy.UPDATED_AT
                    ? 'kitchenDisplayDevice.updated_at'
                    : query.sortBy === KitchenDisplayDeviceSortBy.ID
                      ? 'kitchenDisplayDevice.id'
    ...
    return {
      ...
      data: kitchenDisplayDevices.map((item) =>
        this.formatKitchenDisplayDeviceResponse(item),
      ),
      paginationMeta,
    };
  }
  ...
  async update(
    id: number,
    updateKitchenDisplayDeviceDto: UpdateKitchenDisplayDeviceDto,
    authenticatedUserMerchantId: number,
  ): Promise<OneKitchenDisplayDeviceResponseDto> {
    if (!id || id <= 0) {
      throw new BadRequestException(
    ...
    if (!authenticatedUserMerchantId) {
      throw new ForbiddenException(
        'You must be associated with a merchant to update kitchen display devices',
      );
    }

    const existingKitchenDisplayDevice =
      await this.kitchenDisplayDeviceRepository.findOne({
        where: {
          id,
          merchant_id: authenticatedUserMerchantId,
    ...
    if (updateKitchenDisplayDeviceDto.name !== undefined) {
      if (
        !updateKitchenDisplayDeviceDto.name ||
        updateKitchenDisplayDeviceDto.name.trim().length === 0
      ) {
        throw new BadRequestException('Name cannot be empty');
      }
      ...
      existingKitchenDisplayDevice.name =
        updateKitchenDisplayDeviceDto.name.trim();
    }
    ...
    if (updateKitchenDisplayDeviceDto.deviceIdentifier !== undefined) {
      ...
      existingKitchenDisplayDevice.device_identifier =
        updateKitchenDisplayDeviceDto.deviceIdentifier;
    }
    ...
    if (updateKitchenDisplayDeviceDto.stationId !== undefined) {
      ...
      existingKitchenDisplayDevice.station_id =
        updateKitchenDisplayDeviceDto.stationId || null;
    }
    ...
    if (updateKitchenDisplayDeviceDto.isOnline !== undefined) {
      existingKitchenDisplayDevice.is_online =
        updateKitchenDisplayDeviceDto.isOnline;
    }

    if (updateKitchenDisplayDeviceDto.lastSync !== undefined) {
      existingKitchenDisplayDevice.last_sync =
        updateKitchenDisplayDeviceDto.lastSync || null;
    }

    const updatedKitchenDisplayDevice =
      await this.kitchenDisplayDeviceRepository.save(
        existingKitchenDisplayDevice,
      );
    ...
    if (!completeKitchenDisplayDevice) {
      throw new NotFoundException(
        'Kitchen display device not found after update',
      );
    }
  ...
  async remove(
    ...
    if (!authenticatedUserMerchantId) {
      throw new ForbiddenException(
        'You must be associated with a merchant to delete kitchen display devices',
      );
    }
    ...
    if (!completeKitchenDisplayDevice) {
      throw new NotFoundException(
        'Kitchen display device not found after deletion',
      );
    }

    if (!completeKitchenDisplayDevice.merchant) {
      throw new NotFoundException(
        'Merchant information not found for kitchen display device',
      );
    }
  ...
  private formatKitchenDisplayDeviceResponse(
    kitchenDisplayDevice: KitchenDisplayDevice,
  ): KitchenDisplayDeviceResponseDto {
    if (!kitchenDisplayDevice.merchant) {
      throw new Error(
        'Merchant is required but not loaded in kitchen display device',
      );
    }
