...
import {
  GetMarketingCouponRedemptionQueryDto,
  MarketingCouponRedemptionSortBy,
} from './dto/get-marketing-coupon-redemption-query.dto';
import {
  MarketingCouponRedemptionResponseDto,
  OneMarketingCouponRedemptionResponseDto,
  PaginatedMarketingCouponRedemptionResponseDto,
} from './dto/marketing-coupon-redemption-response.dto';
...
export class MarketingCouponRedemptionsService {
  ...
  async create(
    ...
    if (!authenticatedUserMerchantId) {
      throw new ForbiddenException(
        'You must be associated with a merchant to create marketing coupon redemptions',
      );
    }
    ...
    const coupon = await this.marketingCouponRepository
      ...
      .where('coupon.id = :couponId', {
        couponId: createMarketingCouponRedemptionDto.couponId,
      })
    ...
    if (!coupon) {
      throw new NotFoundException(
        'Marketing coupon not found or you do not have access to it',
      );
    }
    ...
    const order = await this.orderRepository
      ...
      .where('order.id = :orderId', {
        orderId: createMarketingCouponRedemptionDto.orderId,
      })
      .andWhere('order.merchant_id = :merchantId', {
        merchantId: authenticatedUserMerchantId,
      })
    ...
    const customer = await this.customerRepository.findOne({
      where: {
        id: createMarketingCouponRedemptionDto.customerId,
        merchantId: authenticatedUserMerchantId,
      },
    });
    ...
    marketingCouponRedemption.coupon_id =
      createMarketingCouponRedemptionDto.couponId;
    marketingCouponRedemption.order_id =
      createMarketingCouponRedemptionDto.orderId;
    marketingCouponRedemption.customer_id =
      createMarketingCouponRedemptionDto.customerId;
    marketingCouponRedemption.redeemed_at =
      createMarketingCouponRedemptionDto.redeemedAt
        ? new Date(createMarketingCouponRedemptionDto.redeemedAt)
        : new Date();
    marketingCouponRedemption.discount_applied =
      createMarketingCouponRedemptionDto.discountApplied;
    marketingCouponRedemption.status = MarketingCouponRedemptionStatus.ACTIVE;

    const savedMarketingCouponRedemption =
      await this.marketingCouponRedemptionRepository.save(
        marketingCouponRedemption,
      );

    // Fetch the complete marketing coupon redemption with relations
    const completeMarketingCouponRedemption =
      await this.marketingCouponRedemptionRepository.findOne({
    ...
    if (!completeMarketingCouponRedemption) {
      throw new NotFoundException(
        'Marketing coupon redemption not found after creation',
      );
    }
    ...
    return {
      ...
      data: this.formatMarketingCouponRedemptionResponse(
        completeMarketingCouponRedemption,
      ),
    };
  }

  async findAll(
    query: GetMarketingCouponRedemptionQueryDto,
    authenticatedUserMerchantId: number | null | undefined,
  ): Promise<PaginatedMarketingCouponRedemptionResponseDto> {
    // Validate user has merchant
    if (!authenticatedUserMerchantId) {
      throw new ForbiddenException(
        'You must be associated with a merchant to access marketing coupon redemptions',
      );
    }
    ...
    if (query.redeemedDate) {
      ...
      if (!dateRegex.test(query.redeemedDate)) {
        throw new BadRequestException(
          'Redeemed date must be in YYYY-MM-DD format',
        );
      }
    }
    ...
    queryBuilder.andWhere('redemption.status != :deletedStatus', {
      deletedStatus: MarketingCouponRedemptionStatus.DELETED,
    });

    if (query.couponId) {
      queryBuilder.andWhere('redemption.coupon_id = :couponId', {
        couponId: query.couponId,
      });
    }

    if (query.orderId) {
      queryBuilder.andWhere('redemption.order_id = :orderId', {
        orderId: query.orderId,
      });
    }

    if (query.customerId) {
      queryBuilder.andWhere('redemption.customer_id = :customerId', {
    ...
    if (query.redeemedDate) {
      ...
      queryBuilder.andWhere('redemption.redeemed_at >= :startDate', {
        startDate,
      });
    ...
    if (query.createdDate) {
      ...
      endDate.setDate(endDate.getDate() + 1);
      queryBuilder.andWhere('redemption.created_at >= :startDate', {
        startDate,
      });
    ...
    if (query.sortBy) {
      const sortField =
        query.sortBy === MarketingCouponRedemptionSortBy.REDEEMED_AT
          ? 'redemption.redeemed_at'
          : query.sortBy === MarketingCouponRedemptionSortBy.DISCOUNT_APPLIED
            ? 'redemption.discount_applied'
            : query.sortBy === MarketingCouponRedemptionSortBy.CREATED_AT
              ? 'redemption.created_at'
              : query.sortBy === MarketingCouponRedemptionSortBy.UPDATED_AT
    ...
    const [marketingCouponRedemptions, total] =
    ...
    return {
      ...
      data: marketingCouponRedemptions.map((redemption) =>
        this.formatMarketingCouponRedemptionResponse(redemption),
      ),
      paginationMeta,
    };
  }
  ...
  async findOne(
    ...
    if (!id || id <= 0) {
      throw new BadRequestException(
        'Marketing coupon redemption ID must be a valid positive number',
      );
    }
    ...
    const marketingCouponRedemption =
      await this.marketingCouponRedemptionRepository
        .createQueryBuilder('redemption')
        .leftJoinAndSelect('redemption.coupon', 'coupon')
        .leftJoinAndSelect('redemption.order', 'order')
        .leftJoinAndSelect('redemption.customer', 'customer')
        ...
        .andWhere('redemption.status != :deletedStatus', {
          deletedStatus: MarketingCouponRedemptionStatus.DELETED,
        })
        .getOne();
  ...
  async update(
    id: number,
    updateMarketingCouponRedemptionDto: UpdateMarketingCouponRedemptionDto,
    authenticatedUserMerchantId: number | null | undefined,
  ): Promise<OneMarketingCouponRedemptionResponseDto> {
    // Validate ID
    if (!id || id <= 0) {
    ...
    if (!authenticatedUserMerchantId) {
      throw new ForbiddenException(
        'You must be associated with a merchant to update marketing coupon redemptions',
      );
    }

    // Find existing marketing coupon redemption
    const existingMarketingCouponRedemption =
      await this.marketingCouponRedemptionRepository
        .createQueryBuilder('redemption')
        .leftJoinAndSelect('redemption.coupon', 'coupon')
        .leftJoinAndSelect('redemption.order', 'order')
    ...
    if (updateMarketingCouponRedemptionDto.couponId !== undefined) {
      const coupon = await this.marketingCouponRepository
        .createQueryBuilder('coupon')
        .where('coupon.id = :couponId', {
          couponId: updateMarketingCouponRedemptionDto.couponId,
        })
        .andWhere('coupon.merchant_id = :merchantId', {
          merchantId: authenticatedUserMerchantId,
        })
    ...
    if (updateMarketingCouponRedemptionDto.orderId !== undefined) {
      const order = await this.orderRepository
        .createQueryBuilder('order')
        .where('order.id = :orderId', {
          orderId: updateMarketingCouponRedemptionDto.orderId,
        })
        .andWhere('order.merchant_id = :merchantId', {
          merchantId: authenticatedUserMerchantId,
        })
        .andWhere('order.status != :deletedStatus', {
          deletedStatus: 'deleted',
        })
        .getOne();
    ...
    if (updateMarketingCouponRedemptionDto.customerId !== undefined) {
      const customer = await this.customerRepository.findOne({
        where: {
          id: updateMarketingCouponRedemptionDto.customerId,
          merchantId: authenticatedUserMerchantId,
        },
      });
    ...
    if (
      updateMarketingCouponRedemptionDto.discountApplied !== undefined &&
      updateMarketingCouponRedemptionDto.discountApplied < 0
    ) {
      throw new BadRequestException(
        'Discount applied must be greater than or equal to 0',
      );
    ...
    if (updateMarketingCouponRedemptionDto.orderId !== undefined)
      updateData.order_id = updateMarketingCouponRedemptionDto.orderId;
    if (updateMarketingCouponRedemptionDto.customerId !== undefined)
      updateData.customer_id = updateMarketingCouponRedemptionDto.customerId;
    if (updateMarketingCouponRedemptionDto.redeemedAt !== undefined)
      updateData.redeemed_at = new Date(
        updateMarketingCouponRedemptionDto.redeemedAt,
      );
    if (updateMarketingCouponRedemptionDto.discountApplied !== undefined)
      updateData.discount_applied =
        updateMarketingCouponRedemptionDto.discountApplied;

    await this.marketingCouponRedemptionRepository.update(id, updateData);

    // Fetch updated marketing coupon redemption
    const updatedMarketingCouponRedemption =
      await this.marketingCouponRedemptionRepository.findOne({
        where: { id },
        relations: ['coupon', 'order', 'customer'],
      });
    ...
    if (!updatedMarketingCouponRedemption) {
      throw new NotFoundException(
        'Marketing coupon redemption not found after update',
      );
    }
    ...
    return {
      ...
      data: this.formatMarketingCouponRedemptionResponse(
        updatedMarketingCouponRedemption,
      ),
    };
  }
  ...
  async remove(
    ...
    if (!authenticatedUserMerchantId) {
      throw new ForbiddenException(
        'You must be associated with a merchant to delete marketing coupon redemptions',
      );
    }
    ...
    if (
      existingMarketingCouponRedemption.status ===
      MarketingCouponRedemptionStatus.DELETED
    ) {
      throw new ConflictException(
        'Marketing coupon redemption is already deleted',
      );
    }
    ...
    existingMarketingCouponRedemption.status =
      MarketingCouponRedemptionStatus.DELETED;
    await this.marketingCouponRedemptionRepository.save(
      existingMarketingCouponRedemption,
    );
  ...
  private formatMarketingCouponRedemptionResponse(
    marketingCouponRedemption: MarketingCouponRedemption,
  ): MarketingCouponRedemptionResponseDto {
    return {
